...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName('inf-553-2a').getOrCreate()
    ...
    marketBaskets = dataset.map(lambda x: (int(x[0]), int(x[1]))).groupByKey().mapValues(set)

    if case == 2:
        marketBaskets = dataset.map(lambda x: (int(x[1]), int(x[0]))).groupByKey().mapValues(set)
